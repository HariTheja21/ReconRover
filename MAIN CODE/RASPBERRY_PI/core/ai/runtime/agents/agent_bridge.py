...
class AgentBridge:
    ...
    def publish_event(self, event_type: str, event_dict: dict):
        ...
        try:
            topic = "agent.runtime"
            if event_type in ["AgentExecutionStarted", "AgentExecutionCompleted", "AgentConflictDetected", "ConsensusReached", "BlackboardUpdated"]:
                topic = "agent.execution"
            elif event_type in ["AgentStatisticsUpdated", "AgentHealthUpdated"]:
